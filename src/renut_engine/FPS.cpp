...
void appMainDrawend() {
    ...
    // Hooked from config/renut_hooks.toml, address 0x82222250 (appMainDraw's
    // own entry point per config/renut_funcs.toml) with after_instruction =
    // true -- fires once per real call into appMainDraw. Real fix
    // (2026-08-19): game_activity_stats::EndFrame() (the only thing that
    // copies the per-event Record*/Finish* counters -- see
    // render_hooks_stub.cpp -- into the snapshot the Performance tab's "Game
    // activity" section reads) needs a real hook to call it from, or the
    // displayed snapshot never advances past its initial all-zero state even
    // though the counters underneath are incrementing correctly. This
    // address only fires for ~62% of real frames in practice (not confirmed
    // why -- possibly a guest tick-rate/frame-pacing mismatch), so the
    // snapshot updates in bursts rather than every frame, but that's real,
    // working data instead of permanently dead.
